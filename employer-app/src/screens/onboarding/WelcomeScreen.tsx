...
import { AmbientOrbs, LiveDot, PressScale } from '../../components/KlokdLayout';
...
//
// Phones get a purpose-built single-viewport welcome: pitch, swipeable
// proof metrics, primary CTA in the thumb zone. The full marketing page
// stays web-only.

const M_METRICS = [
  { k: '94.2%', l: 'worker show-up rate' },
  { k: '12 min', l: 'median time to fill' },
  { k: 'KES 38M+', l: 'paid via escrow' },
  { k: '2,847', l: 'vetted workers' },
];
    ...
    <View style={m.root}>
      ...
      <FadeUp delay={0} style={[m.top, { paddingTop: insets.top + 14 }]}>
        ...
        <View style={m.forBadge}>
          <Text style={m.forBadgeText}>FOR EMPLOYERS</Text>
        </View>
      </FadeUp>
      ...
      <View style={m.hero}>
        <FadeUp delay={90}>
          <View style={m.livePill}>
            <LiveDot />
            <Text style={m.livePillText}>2,847 VETTED WORKERS READY TONIGHT</Text>
          </View>
        </FadeUp>
        ...
        <FadeUp delay={160}>
          <Text style={m.h1}>
            Hire the shift.{'\n'}
            <Text style={m.h1Accent}>Not the{'\n'}headache.</Text>
          </Text>
        </FadeUp>

        <FadeUp delay={240}>
          <Text style={m.sub}>
            Post a shift, fund the escrow, pick a vetted worker. Money
            releases on clock-out — KRA-compliant by default.
          </Text>
        </FadeUp>
      </View>
      ...
      <FadeUp delay={390} style={m.trustRow}>
        ...
        <Text style={m.trustText}>47 Nairobi employers filling shifts on Klokd</Text>
      </FadeUp>

      {/* CTA block — thumb zone */}
      <FadeUp delay={460} style={[m.ctaBlock, { paddingBottom: Math.max(insets.bottom, 16) + 8 }]}>
        <GradientBtn onPress={() => navigation.navigate('BusinessVerify')}>Post your first shift</GradientBtn>
        <PressScale onPress={() => navigation.navigate('BusinessVerify')} style={m.signInBtn}>
