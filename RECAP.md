...
| **Helpan AI** (agent runtime · briefings · authorities · action dispatch) | ✅ on Railway (`helpanai-production`, Supabase `oklovgfjrnsfxrlcfapg`) · `helpan-klokd-v1` agent admitted + 3 `klokd.*` catalogue scopes (confirmed 25 Jul; safety_policy[klokd] still absent) · env vars set (HELPAN_API_BASE/APP_SECRET/WEBHOOK_SECRET) | 🟡 **Klokd side VERIFIED, blocked on rail 500s (25 Jul)** — HMAC auth + customer JWT both accepted by the live rail; `getHelpanCustomerJwt` (Identiti 0.1.4 session-shaped token) 6/6; inbound match-webhook end-to-end 3/3 on prod; `createBriefing` account_uuid/app_id body fix (`765a2ee`). ⚠️ **deployed rail returns `500 INTERNAL_ERROR` on EVERY write (briefings/events/authorities)** — rail-side (auth+validation+provisioning all pass; RLS ruled out — events_ingested has no RLS); Helpan traces via request_ids. Money leg (→KP) **HELD**. |
...
| Issue | Status |
|---|---|
| `HELPAN_API_BASE` + `HELPAN_APP_SECRET` + `HELPAN_WEBHOOK_SECRET` | ✅ **Set on Klokd Railway (25 Jul)** — HMAC auth + inbound webhook verified live. Remaining Helpan-side: fix the write-path `500`, add `safety_policy[klokd]`, provision `webhookDelivery` worker/URL |
| Customer JWT briefings — Klokd's JWT ≠ Identiti's | ✅ **RESOLVED 24 Jul** — Identiti `0.1.4` audience-aware mint (`POST /v1/customers/{uuid}/tokens`, session-shaped for Helpan); `getHelpanCustomerJwt` wired (`69aadea`), verified 6/6 |
