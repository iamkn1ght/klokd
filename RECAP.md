...
- EAS Update: both apps on `preview` channel + branch (runtime `exposdk:54.0.0`). The 08 Jul APKs embed the updates URL, so JS-only fixes can be shipped over the air via `eas update --branch preview` without a rebuild/reinstall. Also the iOS demo path (Expo Go; standalone iOS + TestFlight deferred until Apple Developer account). Build note: use `EAS_SKIP_AUTO_FINGERPRINT=1` on this machine — local fingerprinting OOMs under ~1 GB free RAM.
...
*Klokd v3 · RECAP v1.4 · 08 July 2026 · Confidential · Update at every sprint boundary.*
